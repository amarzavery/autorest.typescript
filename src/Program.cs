...
namespace AutoRest.TypeScript
{
    ...
    public class Program : NewPlugin
    {
        ...
        protected override async Task<bool> ProcessInternal()
        {
            ...
            Settings.Instance.CustomSettings.Add("ApplyPrefix", await GetValue<bool?>("apply-prefix") ?? false);
            ...
            using (plugin.Activate())
            {
                ...
                {
                    if (await GetValue<bool?>("apply-prefix") ?? false)
                    {
                        plugin.CodeGenerator.GenerateSamplesWithPrefix(codeModel).GetAwaiter().GetResult();
                    }
                    else
                    {
